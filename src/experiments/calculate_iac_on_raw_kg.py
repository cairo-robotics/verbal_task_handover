...
def run_metrics_for_pid(
    ...
    *,
    ...
    kg_path = reconciled_kg

    if not kg_path or not kg_path.exists():
        print(f"    Warning: KG not found for raw graph at {kg_path}. Skipping.")
        return

    # 2. Run IAC
    # iac_output = iac_dir / f"{pid}_{r_type}_iac.json"
    iac_output = kg_dir / f"{pid}_raw_iac.json"
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Calculate IAC for raw KGs for each participant ID using existing KG files."
    )
    parser.add_argument(
        "pids",
        nargs="*",
