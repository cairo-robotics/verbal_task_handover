...
def _format_state_relation(rel: StateRelation) -> str:
    return f"{rel.subject} {rel.relation.value} {rel.object}"


def _miscellaneous_state_relations_for_room(
    room_id: str, state_relations: List[StateRelation]
) -> List[str]:
    lines: List[str] = []
    for rel in state_relations:
        if rel.subject != room_id and rel.object != room_id:
            continue
        if rel.relation == RelationType.LOCATED_IN and rel.object == room_id:
            continue
        lines.append(_format_state_relation(rel))
    return sorted(lines)
...
def _miscellaneous_state_relations_for_character(
    ...
    lines: List[str] = []
    for rel in state_relations:
        if rel.subject != character_id and rel.object != character_id:
            continue
        ...
        characters_present = [
            CharacterView(
                ...
                miscellaneous_state_relations=_miscellaneous_state_relations_for_character(
                    char_id, player_id, extraction.state_relations
                ),
            )
        ...
        room_views.append(
            RoomView(
                ...
                miscellaneous_state_relations=_miscellaneous_state_relations_for_room(
                    room_id, extraction.state_relations
                ),
            )
        )
