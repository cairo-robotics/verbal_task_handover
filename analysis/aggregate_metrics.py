...
# Paths
BASE_DIR = "/media/kaleb/T7/handover_project/v2_data/analysis/metrics_output/"
IAC_DIR = os.path.join(BASE_DIR, "iac")
PR_DIR = os.path.join(BASE_DIR, "precision_recall")
OUTPUT_CSV = os.path.join(BASE_DIR, "aggregated_metrics.csv")
...
def parse_filename(filename, suffix):
    ...
    Example: ari_test_full_realization_iac.json
    """
    name = filename.replace(suffix, "")
    for cond in CONDITIONS:
        if name.endswith(cond):
            # The underscore is before the condition
            pid = name[:-(len(cond) + 1)]
            return pid, cond
    return None, None
...
def main():
    ...
    if os.path.exists(IAC_DIR):
        ...
        print(f"Found {len(iac_files)} IAC files.")
        ...
        print(f"Directory not found: {IAC_DIR}")

    # 2. Process PR files
    if os.path.exists(PR_DIR):
        pr_files = glob.glob(os.path.join(PR_DIR, "*_pr.json"))
        print(f"Found {len(pr_files)} Precision-Recall files.")
        ...
        print(f"Directory not found: {PR_DIR}")
    ...
    print(f"Successfully aggregated metrics into {OUTPUT_CSV}")
